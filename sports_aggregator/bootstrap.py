...
* refresh  -- current-season data that changes regularly.
...
Each step runs in its own subprocess.  That is especially important for history:
a slow or failed season cannot consume the timeout budget for every other year.
"""
...
def steps(season: int) -> list[Step]:
    ...
    # Historical player statistics are deliberately one process per year.  A
    # timeout affects only that season and the command is naturally resumable.
    for historical_year in range(season - 7, season - 1):
        plan.append(Step(
            ...
            ["sports_aggregator.cfb.cli", "backfill", "--year", year,
             "--from-year", str(historical_year), "--to-year", str(historical_year)],
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--timeout", type=int, default=1800,
                        help="Per-step timeout in seconds (history gets this budget per year)")
