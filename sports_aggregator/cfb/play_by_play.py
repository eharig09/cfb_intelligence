...
SCRIMMAGE_WORDS = ("rush", "pass", "sack")
TURNOVER_WORDS = ("interception", "fumble recovery", "fumble return")
...
def _down_type(down: int | None, distance: int | None) -> str | None:
    if down is None or distance is None or down < 1 or down > 4:
        return None
    # Common analytics convention: 2nd-and-8+ and 3rd/4th-and-5+ are passing downs.
...
def _success(down: int | None, distance: int | None, gained: int | None) -> int | None:
    ...
    if threshold is None:
        return None
    return int(gained >= distance * threshold)


def _line_yards(rush_pass: str | None, gained: int | None) -> float | None:
    """Football Outsiders-style line-yards transform, useful without snap grades."""
...
def _garbage(game: dict[str, Any], play: dict[str, Any]) -> int:
    period = _int(play.get("period")) or 0
    a = _int(play.get("offenseScore")) or 0
    d = _int(play.get("defenseScore")) or 0
    margin = abs(a - d)
    # Transparent v1 heuristic. It is deliberately stored/versioned so it can
    # be replaced by a win-probability definition later.
    return int((period >= 4 and margin >= 22) or (period == 3 and margin >= 29))
...
def normalize_play(raw: dict[str, Any], *, season: int, week: int) -> dict[str, Any]:
    ...
    return {
        ...
        "game_id": _int(raw.get("gameId")), "drive_id": str(raw.get("driveId") or "").strip() or None,
        "drive_number": _int(raw.get("driveNumber")), "play_number": _int(raw.get("playNumber")),
        "offense": str(raw.get("offense") or "").strip(), "defense": str(raw.get("defense") or "").strip(),
...
def derive_week(repository, *, season: int, week: int, metric_version: str = METRIC_VERSION) -> dict[str, int]:
    ...
    with closing(repository._connect()) as connection:
        ...
        for row in plays:
            ...
            gained = _int(row.get("yards_gained"))
            ytg = _int(row.get("yards_to_goal"))
            metric_rows.append((
                ...
                _garbage({}, {"period": row.get("period"), "offenseScore": row.get("offense_score"),
                              "defenseScore": row.get("defense_score")}),
        ...
        connection.execute("DELETE FROM cfb_drive_metrics WHERE metric_version=? AND game_id IN (SELECT DISTINCT game_id FROM cfb_plays WHERE season=? AND week=?)",
        ...
        for (game_id, drive_id), group in grouped.items():
            scrimmage = []
            for play in group:
                m = metrics_by_id.get(play["play_id"])
                if m and m[2] in {"rush", "pass"}:
                    scrimmage.append((play, m))
            successes = [m[4] for _, m in scrimmage if m[4] is not None]
            start = group[0]; end = group[-1]
            # Score fields are offense-relative and can switch at possession
            # changes, so drive points are safest when the drive's scoring play
            # is directly represented; richer drive endpoints can refine this.
            points = 0
            for play in group:
                if play.get("scoring"):
                    text = str(play.get("play_type") or "").casefold()
                    points += 3 if "field goal" in text else (2 if "two" in text else 6)
            drive_rows.append((
...
def sync_recent_plays(repository, client, *, season: int, recent_weeks: int = 2,
    ...
    initialize(repository)
    with closing(repository._connect()) as connection:
        weeks = [row[0] for row in connection.execute(
    ...
    for week in sorted(weeks):
        ttl = LIVE_WEEK_TTL if week == max(weeks) else FINISHED_WEEK_TTL
        raw = client.plays(season, int(week), force=force, cache_ttl_seconds=ttl)
...
def game_advanced_summary(repository, game_id: int, *, metric_version: str = METRIC_VERSION) -> dict[str, Any]:
    ...
    for team, items in by_team.items():
        ...
        teams[team] = {
            ...
            "line_yards_per_rush": (sum(float(r["line_yards"]) for r in rush_plays if r.get("line_yards") is not None) /
                                    len([r for r in rush_plays if r.get("line_yards") is not None])) if any(r.get("line_yards") is not None for r in rush_plays) else None,
            "provider_ppa_per_play": (sum(float(r["provider_ppa"]) for r in competitive if r.get("provider_ppa") is not None) /
                                      len([r for r in competitive if r.get("provider_ppa") is not None])) if any(r.get("provider_ppa") is not None for r in competitive) else None,
        }
    return {"game_id": game_id, "metric_version": metric_version, "teams": teams, "drives": drives,
            "own_epa_status": "reserved_for_fitted_expected_points_model"}
