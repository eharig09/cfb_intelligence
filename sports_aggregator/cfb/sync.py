            ...
            for item in poll.get("ranks") or []:
                rankings.append(
                    PollRanking(
                        ...
                        season_type=str(snapshot["seasonType"]),
                        week=int(snapshot["week"]),
                        poll=str(poll["poll"]),
                        is_final=bool(poll.get("isFinal")),
                        rank=int(item["rank"]),
                        team_id=optional_int(item.get("teamId")),
    ...
    def sync(
        ...
        *,
        ...
        jobs: list[tuple[str, Callable[[], int]]] = [
            (
                "teams",
                lambda: self.repository.replace_teams(
                    Team.from_cfbd(item)
                    for item in _as_list("teams", self.client.teams(season, force))
                ),
            ),
            (
                "players",
                lambda: self.repository.replace_players(
                    season, (Player.from_cfbd(item, season) for item in _as_list("roster", self.client.roster(season, force)))
                ),
            ),
            (
                "games",
                lambda: self.repository.replace_games(
                    season,
                    (
                        Game.from_cfbd(item)
            ...
            (
                "media",
                lambda: self.repository.update_game_media(
                    _as_list("media", self.client.game_media(season, force))
                ),
            ),
            (
                "records",
                lambda: self.repository.replace_records(
                    season,
                    (
                        item
                        for item in _as_list("records", self.client.records(season, force))
            ...
            (
                "rankings",
                lambda: self.repository.replace_rankings(
                    season,
                    flatten_rankings(_as_list("rankings", self.client.rankings(season, force))),
                ),
            ),
            (
                "team_stats",
                lambda: self.repository.replace_team_stats(
        ...
        if include_advanced:
            jobs.extend(
                [
                    (
                        "advanced_stats",
                        lambda: self.repository.replace_advanced_stats(
                            season,
                            _as_list(
                                "advanced stats",
                                self.client.advanced_team_stats(season, force),
                            ),
                        ),
                    ),
                    (
                        "core_ratings",
                        lambda: self.repository.replace_core_ratings(
                            season,
                            _as_list("CORE ratings", self.client.core_ratings(season, force)),
                        ),
                    ),
                ]
            )
