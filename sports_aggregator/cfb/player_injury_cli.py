...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        ...
        "with_evidence": sum(1 for item in reports if int(item.get("stored") or 0) > 0),
        "new_recruits": sum(1 for item in reports if item.get("skipped") == "new_recruit"),
        "no_injury_evidence": sum(1 for item in reports if item.get("skipped") == "no_injury_evidence"),
    }, indent=2))
    return 0
